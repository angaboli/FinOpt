...
export function HomeScreen({ navigation }: Props) {
  ...
  const selectedAccountId = useAccountsStore(
    ...
    () =>
      transactions
        .filter(
          (tx) =>
            tx.transactionType === "INCOME" &&
            tx.date.startsWith(thisMonthPrefix),
        )
    ...
    () =>
      transactions
        .filter(
          (tx) =>
            tx.transactionType === "EXPENSE" &&
            tx.date.startsWith(prevMonthPrefix),
        )
        .reduce((s, tx) => s + tx.amount, 0),
    ...
    <View style={styles.container}>
      ...
      >
        ...
        {accounts.length > 0 && selectedAccountId ? (
          ...
          <View style={styles.emptyAccountsCard}>
            ...
            <Text style={styles.subtitle}>
              Ajoutez votre premier compte pour commencer le suivi
              multi-comptes.
            </Text>
          </View>
        )}
        ...
        <View style={styles.balanceCard}>
          ...
          <Text
            style={[styles.balance, totalBalance < 0 && styles.balanceNegative]}
          >
...
const styles = StyleSheet.create({
  ...
  metrics: {
    flexDirection: "row",
    gap: finoptTheme.spacing.lg,
    marginTop: finoptTheme.spacing.xl,
  },
  metric: { flex: 1 },
  metricLabel: {
    color: finoptTheme.colors.gray600,
